...
interface OfficialQuotationDocumentProps {
...
export function OfficialQuotationDocument({
  quotation,
  onClose,
  onSendEmail,
  onConvertToInvoice,
  ...
  const activeCompany = React.useMemo(() => {
    const targetCompId = String(quotation.companyId || (quotation as any).company || activeCompanyId || "tech").toLowerCase().trim()
    const targetCompName = String(quotation.companyName || "").toLowerCase().trim()
  ...
  }, [quotation.companyId, (quotation as any).company, quotation.companyName, activeCompanyId, companies])

  // Resolve Issuing Branch
  const activeBranch: Branch | null = React.useMemo(() => {
    const targetBranchId = String(quotation.branchId || (quotation as any).branch_id || "").toLowerCase().trim()
    const targetBranchName = String(quotation.branchName || (quotation as any).branch_name || "").toLowerCase().trim()
    ...
    return matched || null
  }, [quotation.branchId, (quotation as any).branch_id, quotation.branchName, (quotation as any).branch_name, branches])

  // Resolve Issuing Sub-Branch (Partner)
  const activeSubBranch = React.useMemo(() => {
    const targetSbId = String(quotation.subBranchId || (quotation as any).sub_branch_id || "").toLowerCase().trim()
    const targetSbName = String(quotation.subBranchName || (quotation as any).sub_branch_name || "").toLowerCase().trim()
  ...
  }, [quotation.subBranchId, (quotation as any).sub_branch_id, quotation.subBranchName, (quotation as any).sub_branch_name, subBranches])
  ...
  const resolvedSubtitle = activeSubBranch?.subtitle || activeBranch?.subtitle || activeCompany?.subtitle || "ENTERPRISE SOFTWARE, CREATIVE DESIGN & BUSINESS CONSULTING"
  ...
  const grandTotal = quotation.total || 0
  const subtotal = quotation.subtotal !== undefined ? quotation.subtotal : Math.round(grandTotal / 1.18)
  const gstAmount = quotation.tax !== undefined ? quotation.tax : (grandTotal - subtotal)
  ...
  return (
    <div className="official-quotation-modal-root relative flex flex-col w-full max-w-4xl mx-auto my-2 sm:my-4 bg-white text-zinc-900 rounded-2xl shadow-2xl overflow-hidden border border-zinc-200 print:border-none print:shadow-none print:my-0 print:max-w-none print:w-full">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-xs font-bold uppercase tracking-wider text-zinc-100 flex items-center gap-2">
              <span>Commercial Quotation / Price Tender</span>
              <span className="font-mono text-amber-300">#{quotation.number}</span>
            </h3>
            <p className="text-[10px] text-zinc-400">Verified corporate quotation document with GST breakdown &amp; remittance certifications</p>
          </div>
        </div>
      ...
      <div className="w-full overflow-x-auto">
        <div id="printable-quotation" className="min-w-[620px] sm:min-w-0 p-5 sm:p-10 space-y-5 text-xs leading-relaxed bg-white print:min-w-0 print:p-6 print:space-y-4">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-stretch gap-4 border-b-2 border-zinc-900 pb-4">
          ...
          <div className={`w-full sm:w-60 rounded-2xl ${theme.cardHeaderGradient} text-white p-3.5 shadow-md shrink-0 flex flex-col self-start space-y-2.5`}>
            ...
            <div className="space-y-1.5 text-[10px]">
              <div>
                <span className="text-white/80 text-[8.5px] uppercase tracking-wider font-semibold block">QUOTATION NO:</span>
              ...
              <div className="flex items-center gap-2">
                ...
                <strong className="text-amber-200 font-mono text-[9.5px]">
                  {formatQuotationDate(quotation.validTill)}
                </strong>
              </div>
            </div>

            <div className="pt-2 border-t border-white/20 flex items-center justify-between text-[9px] text-teal-100">
              <span>Commercial Tender</span>
              <span className="font-mono">Ref: {quotation.number}</span>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl border border-zinc-200 bg-zinc-50/70">
          <div className="text-[10px] font-bold uppercase tracking-wider text-teal-900 mb-1.5 flex items-center justify-between">
            <span>QUOTATION PREPARED EXCLUSIVELY FOR (CLIENT):</span>
            <span className="text-zinc-500 font-normal">Currency: INR (₹) • Tax: GST (18%)</span>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <h3 className="text-sm font-black text-zinc-950 uppercase">{quotation.customer}</h3>
              {quotation.customerEmail && (
                <p className="text-zinc-600 font-mono text-[11px] mt-0.5">{quotation.customerEmail}</p>
              )}
              {quotation.customerPhone && (
                <p className="text-zinc-600 text-[11px]">{quotation.customerPhone}</p>
              )}
            </div>
            <div className="text-[11px] text-zinc-600 space-y-0.5 sm:text-right">
              {quotation.customerAddress && (
                <p className="leading-snug">{quotation.customerAddress}</p>
              )}
              {quotation.clientGstin && (
                <p><span className="text-zinc-500">Client GSTIN:</span> <strong className="font-mono font-bold text-zinc-900">{quotation.clientGstin}</strong></p>
              )}
              <p>Place of Supply: <strong className="text-zinc-800 font-semibold">{quotation.placeOfSupply || "West Bengal (Code 19)"}</strong></p>
            </div>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse text-xs">
            <thead className={`${theme.tableHeaderBg} uppercase text-[10px] tracking-wider`}>
              <tr>
                ...
                <th className="py-2.5 px-4 font-bold">Description of Goods / Services &amp; Deliverables</th>
                <th className="py-2.5 px-3 text-center font-bold">SAC/HSN</th>
            ...
            <tbody className="divide-y divide-zinc-200 text-zinc-800">
              {quotation.items && quotation.items.length > 0 ? (
                quotation.items.map((it, idx) => {
                  const itemBase = it.total || (it.unitPrice * it.quantity)
                  const itemTax = Math.round(itemBase * 0.18)
                  const itemCgst = Math.round(itemTax / 2)
                  const itemSgst = itemTax - itemCgst
                  const itemGross = itemBase + itemTax

                  return (
                    <tr key={it.id || idx} className="hover:bg-zinc-50/80 transition-colors">
                      <td className="py-3 px-3 text-center font-mono text-zinc-500">{idx + 1}</td>
                      <td className="py-3 px-4">
                        <div className="font-bold text-zinc-950">{it.name}</div>
                        {it.description && (
                          <p className="text-[11px] text-zinc-500 mt-0.5 leading-normal">{it.description}</p>
                        )}
                      </td>
                      ...
                      <td className="py-3 px-3 text-right font-mono">₹{it.unitPrice.toLocaleString("en-IN")}</td>
                ...
                <tr>
                  <td colSpan={9} className="py-6 text-center text-zinc-400">No quotation items configured.</td>
                </tr>
              )}
            </tbody>
            <tfoot className="bg-zinc-50 border-t border-zinc-200 font-semibold text-zinc-700">
              <tr>
                ...
                <td className="py-2.5 px-3 text-center font-mono">{quotation.items?.reduce((acc, it) => acc + it.quantity, 0) || 1}</td>
                <td></td>
            ...
            <div className="p-4 rounded-xl border border-zinc-200 bg-zinc-50/70 space-y-2 text-[11px]">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1 flex-1">
                  ...
                  {resolvedUpiId && (
                    <div className="flex justify-between"><span className="text-zinc-500">UPI ID:</span> <span className="font-mono font-bold text-emerald-700">{resolvedUpiId}</span></div>
                  )}
                </div>
          ...
          <div className="sm:col-span-5 space-y-2 text-xs">
            <div className="p-4 rounded-xl border border-zinc-200 bg-zinc-50 space-y-2">
              ...
              <div className="border-t-2 border-teal-700 pt-2 flex justify-between items-center text-sm font-black text-zinc-950">
                <span className="uppercase text-xs tracking-wide">Total Quotation Value:</span>
            ...
            <div className="p-3 rounded-xl bg-amber-50/80 border border-amber-200 text-[11px] text-amber-900 leading-snug">
              <span className="font-bold block mb-0.5">Commercial Notice:</span>
              <span>Quoted prices are inclusive of 18% GST and standard SLA. Valid for 30 calendar days from issue date.</span>
            </div>
          </div>
        </div>

        {/* 5. Commercial Terms & Conditions */}
        {(quotation.terms || quotation.notes || activeCompany?.quotation_notes || activeCompany?.terms_conditions) && (
          <div className="p-4 rounded-xl border border-zinc-200 bg-zinc-50/60 space-y-1 text-[11px] text-zinc-600">
            <span className="font-bold text-zinc-900 uppercase text-[10px] tracking-wider block">
              Commercial Terms &amp; Conditions / Project Notes:
            </span>
        ...
        <div className="pt-4 border-t-2 border-zinc-300 grid grid-cols-1 sm:grid-cols-2 gap-8 items-end">
          ...
          <div className="p-4 border border-dashed border-zinc-300 rounded-xl space-y-6 bg-zinc-50/40">
            ...
            <div className="pt-6 border-t border-zinc-300 text-center">
              <p className="font-bold text-zinc-900">{quotation.customer}</p>
          ...
          <div className="p-4 border border-zinc-200 rounded-xl bg-zinc-50/60 flex flex-col items-center justify-center text-center relative">
            ...
            <div className="w-full pt-2 border-t border-zinc-300">
              ...
              <p className="text-[9px] text-zinc-400 mt-0.5">This quotation constitutes a binding commercial offer upon receipt of confirmation.</p>
            </div>
          </div>
        </div>
